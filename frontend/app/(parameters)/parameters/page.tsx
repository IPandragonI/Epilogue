...
function RoleBadge({ role }: { role: UserRole }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full border ${styles[role]}`}
    >
...
function InputWrap({
  ...
  return (
    <div
      className={`relative rounded-lg border ${border} bg-base-100 transition-colors`}
    >
      {children}
    </div>
  );
...
export default function SettingsPage() {
  ...
  const [savedProfileSnapshot, setSavedProfileSnapshot] = useState<string>(
    JSON.stringify(INITIAL_PROFILE),
  );
  const [savedNotionSnapshot, setSavedNotionSnapshot] = useState<string>(
    JSON.stringify(INITIAL_NOTION),
  );
  ...
  useEffect(() => {
    ...
    const initFromServer = async () => {
      ...
      try {
          ...
          try {
            ...
            if (payload && typeof payload === "object") {
              if (payload.user) payload = payload.user;
                ...
                payload = payload.data.user ?? payload.data;
            }
            ...
            console.warn(
              "Failed to parse user response as JSON; using useAuth user",
              err,
            );
          }
        }
        ...
        console.warn(
          "Failed to fetch user from API, falling back to useAuth user",
          err,
        );
      }
      ...
      const agencyDisplayName =
        serverUser?.agency?.name ?? user?.agency?.name ?? "";
      if (agencyId && mounted) {
        setAgencyName(agencyDisplayName);
        setAgencyLoading(true);
        try {
          const agRes = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/agency/${agencyId}/users`,
            {
              method: "GET",
              credentials: "include",
            },
  ...
  if (!user) {
    return (
      <div className="flex items-center justify-center min-h-40">
        <p>
          Utilisateur non connecté — veuillez vous connecter pour accéder à
          cette page.
        </p>
      </div>
    );
  ...
  const handleSaveNotion = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/agency/${user.agency.id}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
  ...
  // ── Agency user update ─────────────────────────────────────────────────
  const handleUpdateAgencyUser = async (
    userId: string,
    data: { firstname: string; lastname: string },
  ) => {
    try {
              ...
              <div className="flex flex-col gap-5">
                ...
                <div className="flex items-center gap-3 bg-base-200 border border-base-300 rounded-lg px-4 py-3">
                  ...
                  >
                    ...
                    <path
                      d="M22 18.5c3.2 2.5 4.4 2.3 10.4 1.9l56.6-3.4c1.2 0 .2-1.2-.4-1.4L79.2 8.6c-2.2-1.7-5.2-3.6-10.8-3.2L14.3 9.2c-2.2.2-2.6 1.3-1.7 2.1L22 18.5z"
                      fill="#000"
                    />
                    <path
                      d="M24.7 28.6V84c0 3 1.5 4.1 4.9 3.9l60.6-3.5c3.4-.2 4.3-2.1 4.3-4.8V24.5c0-2.7-1.1-4.1-3.5-3.9l-63 3.6c-2.6.2-3.3 1.5-3.3 4.4z"
                      fill="#fff"
                    />
                    <path
                      d="M61 29.2L43.4 30.3c-1.2.1-1.5.7-1.5 1.6v2.2c0 .9.3 1.3 1.5 1.2l18-1.1v42.6l-18 1.1c-1.2.1-1.5.7-1.5 1.6v2c0 .9.3 1.3 1.5 1.2L61 81.5c1.5-.1 2-1 2-2.2V31.4c0-1.3-.5-2.3-2-2.2z"
                      fill="#000"
                    />
                  </svg>
                ...
                >
                  <InputWrap exists={exists.notionKey}>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNotion({ ...notion, notionKey: e.target.value })
                      }

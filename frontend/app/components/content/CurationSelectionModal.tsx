...
export default function CurationSelectionModal({
    ...
    const toggleSelection = (id: string) => {
        setLocalSelectedIds((currentIds) => {
            ...
            if (currentIds.length >= MAX_SELECTION) {
                return currentIds;
            }
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" onClick={onClose}>
            <div
                className="bg-base-100 border border-base-300 shadow-xl rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden"
            ...
            >
                <div className="px-5 py-4 border-b border-base-300 flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p className="text-sm text-base-content/50 mt-1">
                            Sélectionnez les documents à utiliser pour générer vos idées.
                        </p>
                    </div>
                    <button onClick={onClose} className="btn btn-ghost btn-xs btn-square shrink-0">
                        <X size={16}/>
                    </button>
                </div>
                ...
                <div className="px-5 py-4 border-b border-base-300 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        <button
                            onClick={() => setLocalSelectedIds(items.slice(0, 8).map((item) => item.id))}
                ...
                <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-2">
                    {filteredItems.length ? filteredItems.map((item) => {
                        ...
                        return (
                            <label
                                key={item.id}
                                className={`flex items-start gap-3 rounded-xl border px-4 py-3 cursor-pointer transition-colors ${checked ? "border-accent bg-base-200" : "border-base-300 bg-base-100 hover:bg-base-200"}`}
                            >
                                <input
                                    type="checkbox"
                                    checked={checked}
                                ...
                                <div className="min-w-0 flex-1">
                                    ...
                                    {item.summary && (
                                        <p className="text-xs text-base-content/55 line-clamp-2 mt-1">
                                            {item.summary}
                                        </p>
                                    )}
                                    {item.source?.name && (
                                        <p className="text-xs text-base-content/35 mt-2 line-clamp-1">
                                            {item.source.name}
                                        </p>
                                    )}
                                </div>
                ...
                <div className="px-5 py-4 border-t border-base-300 flex items-center justify-between gap-3">
                    <p className="text-sm text-base-content/50">
                        {localSelectedIds.length}/{MAX_SELECTION} ressource{localSelectedIds.length > 1 ? "s" : ""} sélectionnée{localSelectedIds.length > 1 ? "s" : ""}
                    </p>
                    <div className="flex items-center gap-2">
                        <button onClick={onClose} className="btn btn-ghost btn-sm">
                            Annuler
                        </button>
                        <button onClick={() => onApply(localSelectedIds)} className="btn btn-accent btn-sm rounded-lg">
                            Appliquer
                        </button>
                    </div>
                </div>
